...
export function useDataTableDetail<TData, TValue>({
  columns,
  filterFields = [],
  ...
  // Search params
  const { page, limit, sort } = searchParamsSchema.parse(
    Object.fromEntries(searchParams),
  );
  const [column, order] = sort?.split(".") ?? [];

  const fixedValue = isFixed
    ? {
        right: ["actions"],
      }
    : {};
  ...
  const [columnPinning, setColumnPinning] = React.useState<{
    left?: string[];
    right?: string[];
  }>(!isEmpty(fixedCol) ? fixedCol : fixedValue);
    ...
    return Array.from(searchParams.entries()).reduce<ColumnFiltersState>(
      (filters, [key, value]) => {
        const filterableColumn = filterableColumns.find(
          (column) => (column as unknown as { value: string }).value === key,
        );
      ...
      JSON.stringify(
        columnFilters.filter((filter) => {
          return searchableColumns.find(
            (column) =>
              (column as unknown as { value: string }).value === filter.id,
          );
        }),
      ),
      500,
  ...
  const filterableColumnFilters = columnFilters.filter((filter) => {
    return filterableColumns.find(
      (column) => (column as unknown as { value: string }).value === filter.id,
    );
  });
