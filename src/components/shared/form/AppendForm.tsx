...
const AppendForm = ({ form, section, icon: Icon, className, name, formControl }: AppendFormProps) => {
  ...
  React.useEffect(() => {
    if (fields.length > collapseStates.length) {
      const missingStates = Array(fields.length - collapseStates.length).fill(true);
  ...
  const toggleCollapse = (index: number) => {
    setCollapseStates((prev) => prev.map((state, i) => (i === index ? !state : state)));
  };
  ...
  return (
    <Form {...formControl}>
      {section && (
        <div className="mt-[25px] mb-4 py-4 text-xl font-semibold">
      ...
      <div className="w-full space-y-2">
        {fields?.map((_field, index) => {
          ...
          return (
            <div className="relative col-span-4 rounded-md border p-4" key={`field-${index}`}>
              <div className="flex items-center justify-between">
                ...
                <Button variant="ghost" disabled={index === 0} onClick={() => handleRemove(index)} type="button">
                  <Trash className="text-red-600" size={16} />
                </Button>
              </div>

              {isExpanded && (
                <div className={`mt-4 ${className ? className : "grid grid-cols-4 gap-4"}`}>
                  {form.map((item, idx) => {
                    return (
                      <div className={item?.className ?? "col-span-1"} key={`${index}-${item.key}-${idx}`}>
                        <FormField
                          control={formControl?.control}
        ...
        <div className="col-span-4 flex items-center justify-end">
          <LoadingButton label="Add More" handleEvent={handleClickAdd} variant="ghost" />
        </div>
      </div>
    </Form>
  );
