...
export const getWorkingShiftValues = (data?: ITimeShift) => ({
  name: data?.name ?? "",
  description: data?.description ?? "",
  fIn: toShortTime(data?.fIn),
  fOut: toShortTime(data?.fOut),
  sIn: toShortTime(data?.sIn),
  sOut: toShortTime(data?.sOut),
  breakMinutes: data?.breakMinutes ?? 60,
  graceMins: data?.graceMins ?? 10,
});
